...
describe("GET /companies", function () {
  ...
  test("bad request with invalid parameter", async function () {
    const resp = await request(app).get("/companies/?nam=c");
    expect(resp.statusCode).toEqual(400);
  });

  test("bad request with invalid min/max parameters", async function () {
    ...
    expect(resp.body).toEqual({
      company: {
        ...
        jobs: [
          ...
          { id: expect.any(Number), title: 'J3', salary: 3, equity: null }
        ],
        name: "C1",
        description: "Desc1",
        numEmployees: 1,
